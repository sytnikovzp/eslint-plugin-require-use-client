...
module.exports = {
  ...
  create(context) {
    ...
    async function resolveImportPath(importSource) {
      try {
        const importResolver = require('eslint-import-resolver-typescript');
        ...
        const result = await new Promise((resolve) => {
          importResolver.resolve(
            importSource,
            filename,
            { extensions: ['.tsx', '.ts', '.jsx', '.js'], project: cwd },
            (err, res) => {
              if (err) resolve(null);
              else resolve(res);
            }
          ...
          for (const stmt of node.body) {
            ...
            if (
              ...
              if (source.startsWith('.') || source.startsWith('/')) {
                try {
                  const importResolverSync = require('eslint-import-resolver-typescript');
                  const resolved = importResolverSync.resolve(
                    source,
                    filename,
                    {
                      extensions: ['.tsx', '.ts', '.jsx', '.js'],
                      project: context.getCwd(),
                    }
                  );

                  const resolvedPath =
                    resolved && resolved.resolved ? resolved.resolved : null;
